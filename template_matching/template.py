...
def find_diff_framed(theImage, reference, loop_i, loop_j, a_i, a_j, ws):
    ...
    for j in range(math.ceil(a_j - ws), math.ceil(a_j + ws + 1)):
        for i in range(math.ceil(a_i - ws), math.ceil(a_i + ws + 1)):
            im3 = theImage[i:i + h, j:j + w]
            if im3.shape[0] != 0 and im3.shape[1] != 0:
                frame_count += 1
...
def find_diff_framed_hierarchical(theImage, reference, loop_i, loop_j, a_i, a_j, ws):
    ...
    x2, y2 = find_diff_framed(theImage_l1, reference_l1, loop_i, loop_j, a_i // 2 + 2 * x1, a_j // 2 + 2 * y1, 1)

    x2 = x2 - a_i // 2
    y2 = y2 - a_j // 2

    fin_x, fin_y = find_diff_framed(theImage, reference, loop_i, loop_j, a_i + 2 * x2, a_j + 2 * y2, 1)
...
def template_mather(random_string):
    ...
    print("P ", ws, frame_count / count, random_string)
    average_counter += (frame_count / count)
    frame_count = 0
...
for i in range(1, 2):
    ...
    template_mather('l2d')
    template_mather('hr')
    print("P ", window_size, average_counter / 3, "average")
